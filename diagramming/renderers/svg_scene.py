...
class SvgScene:
    ...
    def __init__(self, pad: float = 32.0, scale: float = 1.0, background: Optional[str] = "#ffffff") -> None:
        self.pad = pad
        self.scale = scale if scale > 0 else 1.0
        self._background_fill = background or "#ffffff"
        ...
        self._background_rect: Optional[ET.Element] = None
        self.bounds = Bounds()
    ...
    def set_title(self, text: str) -> None:
        title_node = self.root.find("title")
        if title_node is None:
            title_node = ET.SubElement(self.root, "title")
    ...
    def path(self, d: str, **attrs: str) -> ET.Element:
        # Bounding boxes for paths are approximate; rely on explicit updates where needed.
        attr_map = {"d": d}
        attr_map.update({_normalize_attr(k): str(v) for k, v in attrs.items()})
        return ET.SubElement(self.scene, "path", attr_map)
    ...
    def _finalize(self) -> None:
        ...
        pixel_width = view_w * self.scale
        pixel_height = view_h * self.scale
        self.root.set("viewBox", f"{_fmt(view_x)} {_fmt(view_y)} {_fmt(view_w)} {_fmt(view_h)}")
        self.root.set("width", f"{_fmt(pixel_width)}")
        self.root.set("height", f"{_fmt(pixel_height)}")
        background_attrs = {
            "x": _fmt(view_x),
            "y": _fmt(view_y),
            "width": _fmt(view_w),
        ...
        if self._background_rect is None:
            background = ET.Element("rect", background_attrs)
            # Insert after defs but before main scene group.
            insert_index = list(self.root).index(self.scene)
            self.root.insert(insert_index, background)
            self._background_rect = background
        else:
            for key, value in background_attrs.items():
                self._background_rect.set(key, value)
    ...
    def write(self, path, pretty: bool = True) -> None:
