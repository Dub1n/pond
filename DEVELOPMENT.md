...
- **Arrays**: `array` (legacy alias: `run_between`) supplies axis-map entries for the array space plus per-axis `repeat` blocks; optional `through` blocks validate directionality. Missing axes fall back to component-level relates; run instances (`base#n`) are valid refs/selectors.
...
- Arrays: `array` (alias `run_between`) accepts axis-map entries to define array space plus optional `repeat`/`through` blocks; multi-axis tokens (e.g., `-x+y`) anchor true points in `mode: point` (default).
