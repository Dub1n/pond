...
- Schema loader: axis-map `relate` entries keyed by subject axes with `ref`/`pos`/`gap`/`offset`/`mode`; each axis-map key defines a single plane/edge/point (multi-axis keys are not shorthand for multiple independent plane constraints). Coordinate shorthand is supported in world frame (`cxcy: [0, 0]`, `cz: 150`, `-z: joist_top`) and expands to a world-space offset target. Frames (`world`/`local`/`<component_id>`, with `world`/`local` reserved; legacy `component:<id>` accepted) are applied during placement, including `flush` expansion. Center tokens (`cx`, `cy`, `cz`, `~x`, etc.) are valid in keys/targets. `flush` sugar expands to axis-map entries (`faces: all` default; scalar or per-face inset). `place` embeds per-placement axis-maps directly (no nested `relate`). `relate.orient` can override inferred orientation: use `vector` to align a local axis, or `frame` to inherit another component’s basis, then apply `axis` + `twist` roll.
- Arrays: `array` (alias `run_between`) accepts axis-map entries to define array space plus optional `repeat`/`through` blocks; repeat keys are direction vectors with optional frames; `array.orient` sets the basis for each instance; multi-axis tokens (e.g., `-x+y`) anchor true points in `mode: point` (default).
...
- Linear repeats accept `direction`/`interval`/`span` helpers; planner normalises vectors and derives counts/spacing. Operations (`rotate`, `mirror`, `translate`, `boolean`) run in spec order as soon as their inputs resolve; set `include_generated: true` only when transforming previously created clones. Rectangles may declare `boolean.subtract` targets for declarative cutouts (planner unions repeats/rotations automatically).
