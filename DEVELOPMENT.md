...
- **Schema loader**: axis-map `relate` blocks keyed by subject axes (`+x`, `-x+y`, `cx`, `~x`, etc.) resolve to targets with `ref`/`pos`/`gap`/`offset`/`mode`. Each axis-map key defines a single plane/edge/point (multi-axis keys are not shorthand for multiple independent plane constraints). `flush` sugar expands to axis-map entries; per-placement `place` blocks embed axis-maps directly. Frames (`world`/`local`/`<component_id>`, with `world`/`local` reserved) drive placement; relations honour the chosen frame while preserving gaps/offsets and size inference. Optional `relate.orient` sets a component basis using a direction vector and/or frame with axis/twist controls.
...
- Schema loader: axis-map `relate` entries keyed by subject axes with `ref`/`pos`/`gap`/`offset`/`mode`; each axis-map key defines a single plane/edge/point (multi-axis keys are not shorthand for multiple independent plane constraints). Frames (`world`/`local`/`<component_id>`, with `world`/`local` reserved) are applied during placement, including `flush` expansion. Center tokens (`cx`, `cy`, `cz`, `~x`, etc.) are valid in keys/targets. `flush` sugar expands to axis-map entries (`faces: all` default; scalar or per-face inset). `place` embeds per-placement axis-maps directly (no nested `relate`). `relate.orient` can override inferred orientation: use `vector` to align a local axis, or `frame` to inherit another component’s basis, then apply `axis` + `twist` roll.
