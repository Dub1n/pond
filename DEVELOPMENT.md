...
- Arrays: `array` (alias `run_between`) accepts axis-map entries to define array space plus optional `repeat`/`through` blocks; repeat keys are direction vectors with optional frames; multi-axis tokens (e.g., `-x+y`) anchor true points in `mode: point` (default).
