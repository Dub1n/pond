...
- Relationship specs (`schema: pond-relationship*`) are default. Axis-map `relate` entries map subject axes to targets with `ref`/`pos`/`gap`/`offset`/`mode`; multi-reference axis-maps allow rotated placement; `flush` expands to axis-map entries; `place` embeds per-placement axis-maps. Frames (`world`/`local`/`component:<id>`) are honoured in placement. Arrays use `array` (legacy `run_between`) with axis-map entries plus directional `repeat` vectors (`"x,y,z"`) and optional `through` checks; `array` is the canonical placement block (no `relate` on the same component). `kind: reference` components are geometry-less anchors; size inference fills missing axes and lints conflicts. Selectors (`id`, `id.original`, `id.clones`) work in typed `operations` (rotate/mirror/translate/boolean); mirror reflects across axis-aligned planes while keeping frames right-handed. Collisions respect `DIAGRAM_RELATIONSHIPS_COLLISIONS=error|warn|ignore` and skip footings by default; set `DIAGRAM_RELATIONSHIPS_FAIL_ON_WARN=1` to promote warnings. Checks accept `tolerance` and `on_fail: warn|error|ignore`; DOF warnings surface only when an axis remains unconstrained.
...
- Lint: `python scripts/lint_specs.py`, runs solver + IFC validation, collision reporting (`DIAGRAM_RELATIONSHIPS_COLLISIONS=error|warn|ignore`), selector/coverage checks, mesh digests.
