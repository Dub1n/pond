...
- `diagramming/relationships/` – relationship-first schema loader/lint + solver (opt-in via `schema: pond-relationship*`, feature flag `DIAGRAM_RELATIONSHIPS=1`) built around axis-map relations. Components can be `kind: reference` (geometry-less frames) or solids; relations map subject axes to targets with explicit `ref`/`pos`/`gap`/`offset`/`mode` and frames (`world`/`local`/`component:<id>`). Center tokens (`cx`, `cy`, `cz`, `~x`, etc.) are accepted in keys and targets. Run arrays use `run_between` with axis-map `start`/`end` blocks (components or references, center tokens allowed, `orient: along_run` aligns +X to the span, start/end faces can infer/interpolate size). `flush` sugar expands to axis-map entries with default `faces: all`. Per-placement `place` blocks use inline axis-maps (no nested `relate`). Aggregate selectors (`id`, `id.original`, `id.clones`) work everywhere component lists are allowed, including typed `operations` (rotate/mirror/translate/boolean). Size can be inferred from relation pairs; conflicts lint unless matched. Rotations remap numbered instances and keep gaps stable on same-axis faces.
- Relationship specs supply explicit 3D `size` vectors or rely on inference and can use `run_between` for linear arrays tied to datum/reference faces (e.g., joists/pads around openings).
...
- `python scripts/lint_specs.py [--relationship-only|--legacy-only]` – lint specs, including relationship-first examples; runs the relationship solver + IFC export to enforce mm/deg units, Axis/Body contexts, predefined types, material usages, RelVoids wiring, collision overlaps, and emits mesh digests. Relationship linting also checks axis-map coverage, inferred-size conflicts, and selector validity.
