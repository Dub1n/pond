...
- Unit tests at each layer: schema validation, geometry calculations (asserting numeric spans), and raster checks that verify hidden overlays stay visible while buried fills stay masked. Relationship-first solver regressions (like multi-axis `run_between` point anchors for diagonals) are covered via `unittest`.
