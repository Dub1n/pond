...
- Helpers now resolve frames (`world`/`local`/`component:<id>`) through align/contact/flush clauses, expand `relate_from` and `assembly.linear_bracing`, and orient `run_between orient: along_run` with 3D vectors; `run_between` now uses axis-map `start`/`end` blocks (relate syntax) so faces land without insets and sizes can be inferred/interpolated along the span; relationship specs expect explicit or inferred 3D `size: [x, y, z]` tuples for component solids, and section slices render upright with Z flipped internally.
